...
public Action:sm_rehash(args)
{
  if (enableAdmins) {
    ...
    OnRebuildAdminCache(AdminCache_Groups);
...
public SelectBanIpCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
  ...
  if (serverID == -1)
  {
    FormatEx(Query, sizeof(Query), "INSERT INTO %s_bans (type, ip, name, created, ends, length, reason, aid, adminIp, sid, country) VALUES \
            (1, '%s', '', UNIX_TIMESTAMP(), UNIX_TIMESTAMP() + %d, %d, '%s', (SELECT aid FROM %s_admins WHERE authid = '%s' OR authid REGEXP '^STEAM_[0-9]:%s$' LIMIT 1), '%s', \
...
public SelectAddbanCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
  ...
  if (serverID == -1)
  {
    FormatEx(Query, sizeof(Query), "INSERT INTO %s_bans (authid, name, created, ends, length, reason, aid, adminIp, sid, country) VALUES \
            ('%s', '', UNIX_TIMESTAMP(), UNIX_TIMESTAMP() + %d, %d, '%s', (SELECT aid FROM %s_admins WHERE authid = '%s' OR authid REGEXP '^STEAM_[0-9]:%s$' LIMIT 1), '%s', \
            (SELECT sid FROM %s_servers WHERE ip = '%s' AND port = '%s' LIMIT 0,1), ' ')",
      ...
      if (serverID == -1)
      {
        FormatEx(query, sizeof(query),
          "INSERT INTO %s_bans (ip, authid, name, created, ends, length, reason, aid, adminIp, sid) VALUES  \
            ('%s', '%s', '%s', %d, %d, %d, '%s', (SELECT aid FROM %s_admins WHERE authid = '%s' OR authid REGEXP '^STEAM_[0-9]:%s$' LIMIT 1), '%s', \
            (SELECT sid FROM %s_servers WHERE ip = '%s' AND port = '%s' LIMIT 0,1))",
...
public GroupsDone(Handle:owner, Handle:hndl, const String:error[], any:data)
{
  ...
  new GroupId:curGrp = INVALID_GROUP_ID;
  while (SQL_HasResultSet(hndl) && SQL_MoreRows(hndl))
  {
    SQL_FetchRow(hndl);
    if (SQL_IsFieldNull(hndl, 0))
      continue; // Sometimes some rows return NULL due to some setups
...
{
  #if defined DEBUG
  LogToFile(logFile, "ReadConfig_NewSection(): %x %s %b", smc, name, opt_quotes);
  #endif
  ...
  #if defined DEBUG
  LogToFile(logFile, "ReadConfig_NewSection(): NEW STATE %b", ConfigState);
  #endif
...
public SMCResult:ReadConfig_KeyValue(Handle:smc, const String:key[], const String:value[], bool:key_quotes, bool:value_quotes)
{
  ...
  #if defined DEBUG
  LogToFile(logFile, "ReadConfig_KeyValue(): %x %s %s %b %b", smc, key, value, key_quotes, value_quotes);
  #endif
  switch (ConfigState)
  {
...
{
  #if defined DEBUG
  LogToFile(logFile, "ReadConfig_EndSection(): %x", smc);
  #endif
...
stock UTIL_InsertBan(time, const String:Name[], const String:Authid[], const String:Ip[], const String:Reason[], const String:AdminAuthid[], const String:AdminIp[], iBanType, Handle:Pack)
{
  ...
  if (serverID == -1)
  {
    FormatEx(Query, sizeof(Query), "INSERT INTO %s_bans (ip, authid, name, created, ends, length, reason, aid, adminIp, sid, country, type) VALUES \
            ('%s', '%s', '%s', UNIX_TIMESTAMP(), UNIX_TIMESTAMP() + %d, %d, '%s', IFNULL((SELECT aid FROM %s_admins WHERE authid = '%s' OR authid REGEXP '^STEAM_[0-9]:%s$' LIMIT 1),'0'), '%s', \
...
stock PrepareBan(client, target, time, String:reason[], size)
{
  ...
  if (CreateBan(client, target, time, reason))
  {
    ...
    if (g_FloodControl) {
      ...
      KickClient(target, "%s", bannedSite);
    }
  }
